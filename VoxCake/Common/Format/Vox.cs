...
public static class Vox
{
    ...
    private static uint[] GetData(BinaryReader stream)
    {
        uint[] rowData = null;
        uint[] colors = null;
            ...
            while (stream.BaseStream.Position < stream.BaseStream.Length)
            {
                ...
                else if (chunkName == "RGBA")
                {
                    ...
                    for (int i = 0; i < 256; i++)
                    {
                        ...
                        colors[i] = VoxCake.UColor.RGBAToUint(r, g, b, 255);
                    }
                }
                else
        ...
        for (int i = 0; i < voxelData.Length; i++)
        {
            int voxel = (voxelData[i].x * depth + voxelData[i].y) * height + voxelData[i].z;
            rowData[voxel] = (colors == null ? palette[voxelData[i].color - 1] : colors[voxelData[i].color - 1]);
        }
        int temp = height;
        height = depth;
        depth = temp;
